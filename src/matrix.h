...
struct matrix_t {
    ...
    inline double& el(int i, int j) { return data[i * cols + j]; }
    inline double el(int i, int j) const { return data[i * cols + j]; }
    ...
    inline void write(std::ofstream& fstr) const {
        fstr.write(reinterpret_cast<const char*>(data.data()), data.size() * sizeof(double));
    }
};
